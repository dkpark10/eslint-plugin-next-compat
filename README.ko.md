...
이 플러그인은 클라이언트 컴포넌트를 감지하고 **모든 의존성을 추적**하여 클라이언트 코드를 완벽하게 검사합니다.
...
2. `'use client'` 지시어가 있는 파일을 식별
3. 해당 파일들의 의존성 트리를 분석
4. 식별된 클라이언트 파일에 [eslint-plugin-compat](https://github.com/amilajack/eslint-plugin-compat) 규칙을 적용
